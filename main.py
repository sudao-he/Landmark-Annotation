...
# class MyMainForm(QMainWindow):                 # 动态加载 UI 文件
    def __init__(self, imageDir='', imageDir_2='', outDir='', offset=10):
        ...
        self.input_2.clicked.connect(self.open_input_dir_2)
    ...
    def img_resize(self):
        if self.img.width() < self.layout_width and self.img.height() < self.layout_height:
            x_ = (self.layout_width) / self.img.width()
            y_ = (self.layout_height) / self.img.height()
            if x_ > y_:
        ...
        self.scaled_img_2 = self.img_2.scaled(self.img_width, self.img_height)
    ...
    def open_input_dir(self, input_dir=''):
        if input_dir == '':
            ...
            self.imageDir = QFileDialog.getExistingDirectory(self, 'Please select a folder', input_dir)
        # 判空
        if self.imageDir == '':
    ...
    def open_input_dir_2(self, input_dir=''):
        if input_dir == '':
            self.imageDir_2 = QFileDialog.getExistingDirectory(self, 'Please select a folder', './')
        else:
            self.imageDir_2 = QFileDialog.getExistingDirectory(self, 'Please select a folder', input_dir)
        # 判空
        if self.imageDir_2 == '':
            QMessageBox.information(self, "Tips", "Please select a folder", QMessageBox.Yes)
            return
    ...
    def open_output_dir(self, input_dir=''):
        if input_dir == '':
            ...
            self.outDir = QFileDialog.getExistingDirectory(self, 'Please select a folder', input_dir)
        # 判空
        if self.outDir == '':
            QMessageBox.information(self, "Tips", "Please select a folder", QMessageBox.Yes)
            return
    ...
    def Load(self):
        ...
        self.imagepath_2 = self.imageDir_2 + '/' + self.imageList[self.cur]
    ...
    def paintEvent(self, event):
        ...
        p1.setPen(QPen(QColor(255, 0, 255),
                       5 * self.ratio))  # 设置一个画笔，【5*self.ratio】代表半径，因为要考虑到标注的点的大小也要随着图片的放缩而放缩，所以乘以一个ratio
        ...
        for pos_tmp in self.pos_xy_2:
            # 用画笔再画布上绘制圆点。pos_xy中的点的位置是逻辑正确的位置，但是绘制的时候，要画一个物理正确的位置，即需要跟随图片放缩。关键点加入到pos_xy时接连除了ratio和ori_scale，这里要接连乘上
            p1.drawPoint(int(pos_tmp[0] * self.ratio * self.ori_scale + self.layout_width + self.offset),
                         int(pos_tmp[1] * self.ratio * self.ori_scale))
        p1.end()
        ...
        if self.preview_flag_2:  # 确认是否需要开启预览（在鼠标点击右侧坐标点的时候，preview_flag会被设置为True）
            ...
            for idx in indexs:
                # x和y也需要“逻辑位置错误，物理位置正确”。w和h代表框的边长，同样需要跟随图片的放缩而放缩
                x = int((self.pos_xy_2[idx][0] - 15) * self.ratio * self.ori_scale + self.layout_width + self.offset)
                y = int((self.pos_xy_2[idx][1] - 15) * self.ratio * self.ori_scale)
                w = int(30 * self.ratio * self.ori_scale)
                h = w
        ...
        if event.button() == Qt.LeftButton:  # 鼠标左键按下
            ...
            if self.layout_width + self.offset < event.x() < self.layout_width + self.offset + self.img_width and 0 < event.y() < self.img_height:  # 因为图片被放缩了，如果点击到图片之外的区域，不去处理
                ...
                self.listWidget_2.insertItem(len(self.listWidget_2), '(' + str(
                    round((event.x() - self.layout_width - self.offset) / self.ratio / self.ori_scale, 2)) + ',' + str(
                    round(event.y() / self.ratio / self.ori_scale, 2)) + ')')
                self.pos_xy_2.append(((event.x() - self.layout_width - self.offset) / self.ratio / self.ori_scale,
                                      event.y() / self.ratio / self.ori_scale))
                self.update()
            # else:
...
if __name__ == '__main__':
    ...
    w = MyMainForm(imageDir=r'E:\Sudao-Project\ReDFeat\1211-CYT&TowerD&9&10\test\IR',
                   imageDir_2=r'E:\Sudao-Project\ReDFeat\1211-CYT&TowerD&9&10\test\VIS',
                   outDir=r'E:\fasade_project\1211-CYT&TowerD&9&10\landmark')
